...
export class Bancontact extends BaseComponent {
  ...
  mount(selector: string) {
    ...
    if (!container) {
      console.warn("[Bancontact] Container not found:", safeSelector);
      return;
    }
  ...
  async submit() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error("[Bancontact] HTTP error:", {
  ...
  private _getTemplate() {
    ...
    const description =
      locale.startsWith("de")
        ? "Bezahlen Sie bequem mit Bancontact und schließen Sie die Zahlung sicher ab."
        : "Pay easily with Bancontact and complete your payment securely.";
